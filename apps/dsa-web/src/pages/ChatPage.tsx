...
const QUICK_QUESTIONS = [
  { labelKey: 'chat.quickQ1', skill: 'chan_theory' },
  { labelKey: 'chat.quickQ2', skill: 'wave_theory' },
  { labelKey: 'chat.quickQ3', skill: 'bull_trend' },
  { labelKey: 'chat.quickQ4', skill: 'box_oscillation' },
  { labelKey: 'chat.quickQ5', skill: 'bull_trend' },
  { labelKey: 'chat.quickQ6', skill: 'emotion_cycle' },
] as const;
...
const getMessageSkillLabel = (msg: Message, separator = '、'): string => getMessageSkillNames(msg).join(separator);
...
const ChatPage: React.FC = () => {
  ...
  const skillSeparator = language === 'zh' ? '、' : ', ';
    ...
    async (stockCode: string) => {
      ...
      try {
        ...
        if (existingStockCode) {
          ...
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(t('chat.watchlistRemoved', { code: stockCode }));
          }
        } else {
          const codes = await systemConfigApi.addToWatchlist(stockCode);
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(t('chat.watchlistAdded', { code: stockCode }));
          }
        }
      } catch {
        if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    void systemConfigApi.getConfig(false)
      ...
      .catch((error) => {
        ...
        setContextCompressionError(parsed.message || t('chat.contextCompressionReadFailed'));
  ...
  const handleSend = useCallback(
    async (overrideMessage?: string, overrideSkillIds?: string[]) => {
      ...
      const usedSkillNames = usedSkillIds.length > 0 ? getSkillNames(usedSkillIds) : [t('chat.generalSkill')];
      ...
      await startStream(payload, {
        ...
        skillName: usedSkillNames.join(skillSeparator),
      });
    },
    [activeStockContext, getSkillNames, input, loading, normalizeSelectedSkillIds, requestScrollToBottom, selectedSkillIds, sessionId, skillSeparator, startStream, t],
  );
  ...
  const handleQuickQuestion = (q: (typeof QUICK_QUESTIONS)[number]) => {
    ...
    handleSend(t(q.labelKey), [q.skill]);
  };
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    const skillLabel = getMessageSkillLabel(msg, skillSeparator);
    const heading = msg.role === 'user' ? `# ${t('chat.userMessageHeading')}` : `# ${t('chat.aiReplyHeading')}${skillLabel ? ` · ${skillLabel}` : ''}`;
  ...
  const getCurrentStage = (steps: ProgressStep[]): string => {
    ...
    if (last.type === 'thinking') return last.message || t('chat.aiThinking');
    if (last.type === 'tool_start')
      return `${last.display_name || last.tool}...`;
    if (last.type === 'tool_done')
      return `${last.display_name || last.tool} ${t('chat.toolDoneSuffix')}`;
    if (last.type === 'generating')
      return last.message || t('chat.generatingAnalysis');
  ...
  const renderThinkingBlock = (msg: Message) => {
    ...
    const summary = t('chat.toolCallsSummary', { count: toolSteps.length, duration: totalDuration.toFixed(1) });
    ...
    <div className="mb-3 pl-5 border-l border-border/40 space-y-1.5 animate-fade-in">
      {steps.map((step, idx) => {
        ...
        if (step.type === 'thinking') {
          ...
          text = step.message || t('chat.generating');
            ...
            {sessions.map((s) => (
              <div key={s.session_id} className="session-item-row">
                <button
                  ...
                  aria-label={t('chat.switchSessionAria', { title: s.title })}
                ...
                <button
                  ...
                  aria-label={t('chat.deleteSessionAria', { title: s.title })}
                >
                  <svg
  ...
  const selectedSkillSummary = selectedSkillIds.length > 0
    ? getSkillNames(selectedSkillIds).join(skillSeparator)
        ...
        <header className="mb-4 flex-shrink-0 space-y-3">
          <div className="flex items-start justify-between gap-4">
            ...
            {messages.length > 0 && (
              <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2">
                <Tooltip content={t('chat.exportSessionTooltip')}>
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      onClick={() => downloadSession(messages)}
                      aria-label={t('chat.exportSessionTooltip')}
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
                ...
                <Tooltip content={t('chat.sendNotificationTooltip')}>
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          showSendFeedback({ type: 'success', message: t('chat.sentToNotification') }, 3000);
                        } catch (err) {
                      ...
                      aria-label={t('chat.sendNotificationTooltip')}
                    >
                      {sending ? (
                        <svg
          ...
          {sendToast ? (
            <InlineAlert
              ...
              title={sendToast.type === 'success' ? t('chat.sendSuccessTitle') : t('chat.sendFailedTitle')}
                ...
                >
                  ...
                  >
                    {msg.role === 'assistant' && skillLabel && (
                      <div className="mb-2">
                        <Badge variant="info" className="chat-skill-badge shadow-none" aria-label={t('chat.skillAria', { label: skillLabel })}>
                          <svg
                            className="w-3 h-3"
                            fill="none"
                            stroke="currentColor"
                      ...
                      <div className="relative">
                        <div className="chat-message-actions">
                          <button
                            ...
                            aria-label={copiedMessages.has(msg.id) ? t('common.copied') : t('common.copy')}
                          >
                            {copiedMessages.has(msg.id) ? t('common.copied') : t('common.copy')}
                          </button>
                          <button
                            type="button"
                            onClick={() => downloadMessageAsMarkdown(msg)}
                            className="chat-copy-btn"
                            aria-label={t('chat.exportMessageAria')}
                          >
                            {t('chat.export')}
                          </button>
                        </div>
              ...
              {skills.length > 0 && (
                <div className="space-y-2">
                  <button
                    ...
                    aria-label={mobileSkillPickerOpen ? t('chat.collapseStrategy') : t('chat.expandStrategy')}
            ...
            {activeStockCode && (
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className="text-[11px]"
                >
                  {stockInWatchlist(activeStockCode) ? t('report.removeFromWatchlist') : t('report.addToWatchlist')}
                </Button>
                {watchlistMessage && (
