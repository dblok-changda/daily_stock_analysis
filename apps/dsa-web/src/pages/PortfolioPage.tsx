...
const PortfolioPage: React.FC = () => {
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning(text.brokerEmptyWarning);
      ...
      setBrokers(FALLBACK_BROKERS);
      setBrokerLoadWarning(text.brokerUnavailableWarning);
      if (!FALLBACK_BROKERS.some((item) => item.broker === selectedBroker)) {
        setSelectedBroker(FALLBACK_BROKERS[0].broker);
      }
    }
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || text.riskDegradedMessage);
      }
    } catch (err) {
      setSnapshot(null);
      setRisk(null);
  ...
  const handleAnalyzePosition = async (row: FlatPosition) => {
    ...
    try {
      ...
      setPositionAnalysisMessage(formatUiText(text.analyzeSubmitted, { symbol: row.symbol, taskId: task.taskId }));
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    if (!writableAccountId) {
      setWriteWarning(text.selectAccountFirst);
      return;
    }
    try {
      setWriteWarning(null);
  ...
  const openDeleteDialog = (item: PendingDelete) => {
    if (!writableAccountId) {
      setWriteWarning(text.selectAccountForDelete);
      return;
    }
  ...
  const openAccountDeleteDialog = () => {
    if (!writableAccount) {
      setWriteWarning(text.selectAccountForAccountDelete);
      return;
    }
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAccountCreateError(parsed.message || text.accountCreateFailed);
      setAccountCreateSuccess(null);
    } finally {
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      ...
      {(showCreateAccount || !hasAccounts) ? (
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-foreground">{text.newAccount}</h2>
            {hasAccounts ? (
              <button
                type="button"
              ...
              <span className="text-xs text-secondary">{text.autoSwitchHint}</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
              ...
              title={text.accountCreateFailedTitle}
          ...
          {accountCreateSuccess ? (
            <InlineAlert
              variant="success"
              className="mt-2 rounded-lg px-2 py-1 text-xs shadow-none"
              title={text.accountCreateSuccessTitle}
          ...
          <form className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2" onSubmit={handleCreateAccount}>
            ...
            >
              <option value="cn">{text.marketCn}</option>
              <option value="hk">{text.marketHk}</option>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{text.manualTrade}</h3>
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={text.tradeSymbolPlaceholder} value={tradeForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={tradeForm.side}
                ...
                <option value="buy">{PORTFOLIO_SIDE_LABELS[language].buy}</option>
                <option value="sell">{PORTFOLIO_SIDE_LABELS[language].sell}</option>
              </select>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={text.quantityPlaceholder} value={tradeForm.quantity}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, quantity: e.target.value }))} required />
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={text.pricePlaceholder} value={tradeForm.price}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, price: e.target.value }))} required />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={text.feePlaceholder} value={tradeForm.fee}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, fee: e.target.value }))} />
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={text.taxPlaceholder} value={tradeForm.tax}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, tax: e.target.value }))} />
            </div>
            <p className="text-xs text-secondary">{text.feeTaxHint}</p>
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{text.submitTrade}</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{text.manualCash}</h3>
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={cashForm.direction}
                ...
                <option value="in">{PORTFOLIO_CASH_DIRECTION_LABELS[language].in}</option>
                <option value="out">{PORTFOLIO_CASH_DIRECTION_LABELS[language].out}</option>
              </select>
            </div>
            <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={text.amountPlaceholder}
              value={cashForm.amount} onChange={(e) => setCashForm((prev) => ({ ...prev, amount: e.target.value }))} required />
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={formatUiText(text.currencyPlaceholder, { currency: writableAccount?.baseCurrency || 'CNY' })} value={cashForm.currency}
              onChange={(e) => setCashForm((prev) => ({ ...prev, currency: e.target.value }))} />
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{text.submitCash}</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{text.manualCorporate}</h3>
          <form className="space-y-2" onSubmit={handleCorporateSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={text.corpSymbolPlaceholder} value={corpForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={corpForm.actionType}
                ...
                <option value="cash_dividend">{PORTFOLIO_CORPORATE_ACTION_LABELS[language].cash_dividend}</option>
                <option value="split_adjustment">{PORTFOLIO_CORPORATE_ACTION_LABELS[language].split_adjustment}</option>
              </select>
            </div>
            {corpForm.actionType === 'cash_dividend' ? (
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.000001" placeholder={text.dividendPerSharePlaceholder}
              ...
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.000001" placeholder={text.splitRatioPlaceholder}
                value={corpForm.splitRatio}
                onChange={(e) => setCorpForm((prev) => ({ ...prev, splitRatio: e.target.value, cashDividendPerShare: '' }))} required />
            )}
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{text.submitCorporate}</button>
          </form>
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{text.csvImport}</h3>
          <div className="space-y-2">
            {brokerLoadWarning ? (
              <InlineAlert
                variant="warning"
            ...
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={selectedBroker} onChange={(e) => setSelectedBroker(e.target.value)}>
                {brokers.length > 0 ? (
                  ...
                  <option value="huatai">{text.huataiFallback}</option>
                )}
              </select>
            ...
            {csvParseResult ? (
              <InlineAlert
                ...
                message={formatUiText(text.csvParseResultMessage, { valid: csvParseResult.recordCount, skipped: csvParseResult.skippedCount, errors: csvParseResult.errorCount })}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
            {csvCommitResult ? (
              <InlineAlert
                ...
                title={csvDryRun ? text.csvDryRunResultTitle : text.csvCommitResultTitle}
                message={formatUiText(text.csvCommitResultMessage, { mode: csvDryRun ? text.csvDryRunMode : text.csvCommitMode, inserted: csvCommitResult.insertedCount, duplicate: csvCommitResult.duplicateCount, failed: csvCommitResult.failedCount })}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
          </div>
        ...
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{text.eventRecords}</h3>
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={eventType} onChange={(e) => setEventType(e.target.value as EventType)}>
                <option value="trade">{text.tradeLedger}</option>
                <option value="cash">{text.cashLedger}</option>
                <option value="corporate">{text.corporateLedger}</option>
              </select>
              <button type="button" className="btn-secondary text-sm" onClick={() => void loadEvents()} disabled={eventLoading}>
                {eventLoading ? text.loadingLedger : text.refreshLedger}
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2">
            ...
            {(eventType === 'trade' || eventType === 'corporate') ? (
              <input className={PORTFOLIO_INPUT_CLASS} placeholder={text.filterBySymbol} value={eventSymbol}
            ...
            {eventType === 'trade' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventSide} onChange={(e) => setEventSide(e.target.value as '' | PortfolioSide)}>
                <option value="">{text.allSides}</option>
                <option value="buy">{PORTFOLIO_SIDE_LABELS[language].buy}</option>
                <option value="sell">{PORTFOLIO_SIDE_LABELS[language].sell}</option>
              </select>
            ) : null}
            {eventType === 'cash' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventDirection}
                ...
                <option value="">{text.allCashDirections}</option>
                <option value="in">{PORTFOLIO_CASH_DIRECTION_LABELS[language].in}</option>
                <option value="out">{PORTFOLIO_CASH_DIRECTION_LABELS[language].out}</option>
              </select>
            ) : null}
            {eventType === 'corporate' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventActionType}
                ...
                <option value="">{text.allCorporateActions}</option>
                <option value="cash_dividend">{PORTFOLIO_CORPORATE_ACTION_LABELS[language].cash_dividend}</option>
                <option value="split_adjustment">{PORTFOLIO_CORPORATE_ACTION_LABELS[language].split_adjustment}</option>
              </select>
            ) : null}
            <div className="text-[11px] text-secondary">
              {writeBlocked ? text.deleteBlockedHint : text.deleteHint}
            </div>
              ...
              {eventType === 'trade' && tradeEvents.map((item) => (
                <div key={`t-${item.id}`} className="flex items-start justify-between gap-3 border-b border-white/5 py-2 text-xs text-secondary">
                  <div className="min-w-0">
                    {formatUiText(text.tradeSummary, { date: item.tradeDate, side: formatSideLabel(item.side), symbol: item.symbol, quantity: item.quantity, price: item.price })}
                  </div>
                  {!writeBlocked ? (
                    <button
                      type="button"
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'trade',
                        id: item.id,
                        message: formatUiText(text.deleteTradeConfirm, { date: item.tradeDate, side: formatSideLabel(item.side), symbol: item.symbol, quantity: item.quantity, price: item.price }),
                      })}
                    >
                      {text.deleteLabel}
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'cash',
                        id: item.id,
                        message: formatUiText(text.deleteCashConfirm, { date: item.eventDate, direction: formatCashDirectionLabel(item.direction), amount: item.amount, currency: item.currency || '' }),
                      })}
                    >
                      {text.deleteLabel}
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: formatUiText(text.deleteCorporateConfirm, { date: item.effectiveDate, action: formatCorporateActionLabel(item.actionType), symbol: item.symbol }),
                      })}
                    >
                      {text.deleteLabel}
                    </button>
            ...
            <div className="flex items-center justify-between text-xs text-secondary">
              <span>{formatUiText(text.pageOf, { page: eventPage, total: totalEventPages })}</span>
              <div className="flex gap-2">
      ...
      <ConfirmDialog
        ...
        title={text.deleteLedgerTitle}
        message={pendingDelete?.message || text.deleteLedgerConfirm}
        confirmText={deleteLoading ? t('common.deleting') : text.confirmDeleteLabel}
