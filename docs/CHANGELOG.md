...
- [Docs] Ported README, current guides, integration documentation, CI output, and operator-facing copy to English.
- [Improvement] Documented report-language configuration as an explicit runtime setting.
- [Improvement] Kept compact notification summaries while retaining complete DecisionSignal details in stock detail views and single-stock reports.
- [Improvement] Clarified low-memory deployment expectations and local startup behavior.
- [Fix] Prevented slow `uvicorn` startup from being mistaken for a failed health check.
- [Fix] Improved data-source fallback behavior so transient fetch failures do not repeatedly degrade unrelated fetches.
- [Fix] Corrected portfolio risk snapshot semantics and aggregate notification display gaps.
- [Fix] Redacted sensitive response details from user-facing API errors while keeping diagnostics in logs.
- [Fix] Improved JP/KR symbol handling examples such as `000660`, `005930`, `7203.T`, `005930.KS`, and `035720.KQ`.
